...
def search_arxiv(
    ...
    """
    if not query.strip():
        return ErrorResponse(error="Query cannot be empty", details="Provide a non-empty search query.")
...
def arxiv_search_tool(
    ...
    Args:
        query: Search query (e.g. 'transformer attention', 'au:Goodfellow',
               'cat:cs.AI'). Supports field prefixes: all:, ti:, au:, abs:, cat:.
        max_results: Max results to return (default 10, max 50).
        sort_by: Sort criterion ('relevance', 'submitted_date', 'updated_date').

    Returns:
        Markdown-formatted list of papers with title, authors, date, categories,
        and abstract excerpt. ErrorResponse on failure.
    """
