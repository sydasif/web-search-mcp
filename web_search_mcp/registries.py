...
Provides ``lookup_package`` for fetching a specific package's metadata
and ``search_packages`` for keyword-based discovery across registries.
...
def _client() -> httpx.Client:
    """Return a shared httpx client configured via settings."""
    return httpx.Client(
        timeout=TIMEOUT,
...
def _search_npm(query: str, max_results: int = 5) -> list[PackageInfo]:
    ...
    for item in data.get("objects", []):
        ...
        if not name:
            continue
        # npm search doesn't return download counts; use popularity as a
        # relative indicator (0-1 float derived from downloads + dependents).
...
def _lookup_pypi(name: str) -> PackageInfo | None:
    ...
    # Try to get download count from PyPI stats (redirects, so check common URL)
    downloads_str: str | None = None
    try:
        with _client() as c:
...
def _search_crates(query: str, max_results: int = 5) -> list[PackageInfo]:
    ...
    try:
        with _client() as c:
            resp = c.get(
                "https://crates.io/api/v1/crates", params={"q": query, "per_page": max_results}
            )
            resp.raise_for_status()
            data = resp.json()
    except Exception as e:
...
def _lookup_go(module: str) -> PackageInfo | None:
    ...
    # Go proxy returns { "Version": "v1.2.3", "Time": "2024-01-15T..." }
    return PackageInfo(
        name=module,
        registry="go",
...
def _detect_registry(name: str) -> Registry:
    ...
    - ``github.com/`` paths are Go modules
    - Names containing dots are likely PyPI packages
    - Everything else tries npm first, then crates.io

    Args:
        name: Package name or module path.

    Returns:
        A registry string literal.
    """
    if name.startswith("@"):
        return "npm"
    if re.match(r"^github\.com/", name, re.IGNORECASE):
        return "go"
    # Names with dots are conventionally PyPI (numpy, requests)
    if "." in name:
        return "pypi"
    return "npm"
...
def lookup_package(
    ...
    Args:
        name: Package or module name. Examples: ``"express"``, ``"numpy"``,
            ``"serde"``, ``"github.com/gin-gonic/gin"``.
        registry: One of ``"npm"``, ``"pypi"``, ``"crates"``, ``"go"``.
            Auto-detected from the name when omitted.

    Returns:
        A ``PackageInfo`` object with version, description, downloads, etc.

    Error Handling:
        - Package not found: returns ErrorResponse with details
        - Registry offline: returns ErrorResponse
    """
...
def search_packages(
    ...
    Args:
        ...
        registry: Which registry to search (``"npm"``, ``"pypi"``,
            ``"crates"``, ``"go"``). Defaults to ``"npm"``.
        max_results: Max results to return (1-20).

    Returns:
        A list of ``PackageInfo`` results.
    ...
    try:
        ...
        logger.error("Search failed on %s: %s", registry, e)
